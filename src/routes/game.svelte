			...
			for (let x = 0; x < columns; x++) {
				...
				switch (cell.status) {
					...
					case 'mine':
						...
						bombSprite.width = cellSize; // adjust the sprite to adjust the cell
...
<style>
	...
	.popup {
		display: none; /* 默认不显示，需要时通过JavaScript改变这个属性 */
		position: fixed; /* 固定定位，使得弹出层浮动在页面上 */
		left: 50%; /* 将弹出层的左边缘移至页面中心 */
		top: 10px; /* 距离顶部10像素 */
		transform: translateX(-50%); /* 向左移动自身宽度的50%，以实现真正的水平居中 */
		width: auto; /* 宽度根据内容自动调整 */
		max-width: 300px; /* 最大宽度，确保在小屏幕上也能良好显示 */
		background: rgba(255, 255, 255, 0.95);
		padding: 20px;
		border-radius: 10px;
